    ...
    const SerialJoints& current_joints_tuple)
{
    ...
    double min_max_difference = std::numeric_limits<double>::max(); // 初始化为最大可能值
    SerialJoints closest_solution; // 用于存储最近的解

    // 使用 std::apply 来计算最大差值
    auto calculate_max_difference = [&](const SerialJoints& s1, const SerialJoints& s2) {
        double max_diff = 0.0;
        std::apply([&](auto&&... args1) {
            std::apply([&](auto&&... args2) {
                // 使用折叠表达式计算每个关节的差值，并找出最大值
                double temp_arr[] = { std::abs(args1 - args2)... };
                max_diff = *std::max_element(std::begin(temp_arr), std::end(temp_arr));
            }, s2);
        }, s1);
        return max_diff;
    };
    ...
    // 遍历所有可能的关节解
    for (const auto& solution_tuple : possible_serial_joints_vec) {
        // 计算当前解与当前关节位置之间的最大关节差值
        double current_max_difference = calculate_max_difference(solution_tuple, current_joints_tuple);

        // 如果当前最大差值更小，则更新最小最大差值和最近的解
        if (current_max_difference < min_max_difference) {
            min_max_difference = current_max_difference;
            closest_solution = solution_tuple;
        }
    }
        ...
        const SerialJoints& current_joints_tuple)
{
    ...
    for (const auto& solution_tuple_with_phi : possible_serial_joints_vec_with_phi) {
        ...
        // 如果当前最大差值非常接近当前最小最大差值，则比较臂角
        if (fabs(current_max_difference - min_max_difference) <= EPSILON*1e5) {
            if (std::fabs(current_phi) < min_phi_abs) {
                min_phi_abs = std::fabs(current_phi); // 更新最小phi绝对值
                closest_solution_with_phi = solution_tuple_with_phi; // 更新为当前解
            }
        ...
        // 如果当前最大差值更小，则更新最小最大差值和最近的解
        else if (current_max_difference < min_max_difference) {
            min_max_difference = current_max_difference;
            min_phi_abs = std::fabs(current_phi);
            closest_solution_with_phi = solution_tuple_with_phi;
        }
