...
class TeleopSystemMujoco:
    ...
    def _initialize_full_mode(self):
        ...
        ik_backend = str(
            arm_config.get("ik_backend", "ros_service")
        ).strip().lower()
        if ik_backend == "marvin_local":
            from arm_control.arm_teleop_mujoco_marvin import (
                ArmTeleopMujocoMarvin as ArmTeleopClass,
            )
        else:
            from arm_control.arm_teleop_mujoco import (
                ArmTeleopMujoco as ArmTeleopClass,
            )
        logger.info("机械臂逆解后端: %s", ik_backend)
        self.arm_teleop = ArmTeleopClass(
            self.vp_streamer,
