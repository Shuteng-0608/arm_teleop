...
try:
    ...
    # The Marvin standalone entry intentionally runs without ROS. The local
    # response classes keep internal recording code importable/testable, while
    # actual ROS service publication remains guarded by ROS_AVAILABLE.
    rospy = None
    SetRecording = None
    Trigger = None
    ROS_AVAILABLE = False
...
class RobotControllerMuJoCoPegTool:
    ...
    def activate_runtime(self) -> None:
        ...
        if self.enable_recording_service and self.hdf5_recorder is not None:
            if not ROS_AVAILABLE:
                raise RuntimeError(
                    "enable_recording_service=true requires ROS. Set it to "
                    "false when using the Python-only Marvin entry."
                )
    ...
    def _call_teleop_trigger_service(self, service_name: str, description: str) -> bool:
        ...
        if not ROS_AVAILABLE:
            print(
                "[Episode Teleop] ROS is unavailable. Set "
                "teleop_controlled_by_recording=false for Python-only mode."
            )
            return False
    ...
    def get_body_pose_matrix(self, body_name: str) -> Optional[np.ndarray]:
        """Return an isolated 4x4 world pose for a named MuJoCo body."""
        ...
        with self.lock:
            ...
            pose[:3, :3] = self.data.xmat[body_id].copy().reshape(3, 3)
            pose[:3, 3] = self.data.xpos[body_id].copy()
            return pose
