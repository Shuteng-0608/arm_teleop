...
def _deep_merge(base: Mapping[str, Any], override: Mapping[str, Any]) -> Dict[str, Any]:
    """Recursively merge a small profile override onto a base config."""
...
def _normalize_declared_paths(
    ...
    """Resolve paths in the file that declared them before profile merging."""
    normalized = deepcopy(dict(config))
    for key in ("mujoco_model_path", "record_dir", "hdf5_record_dir"):
        if normalized.get(key):
            normalized[key] = _resolve_path(normalized[key], config_dir)

    if isinstance(normalized.get("arm_config"), Mapping):
        arm_config = dict(normalized["arm_config"])
        for key in (
            "marvin_ik_module_path",
            "marvin_ik_library_path",
            "marvin_ik_library_path_windows",
            "marvin_ik_library_path_linux",
            "marvin_ik_config_path",
        ):
            if arm_config.get(key):
                arm_config[key] = _resolve_path(arm_config[key], config_dir)
        normalized["arm_config"] = arm_config
    return normalized


def _load_config_mapping(path: Path, stack: Sequence[Path]) -> Dict[str, Any]:
    """Load one config and its optional ``extends`` chain."""
    if path in stack:
        chain = " -> ".join(str(item) for item in (*stack, path))
        raise MujocoConfigError(f"Config extends cycle detected: {chain}")

    try:
        with path.open("r", encoding="utf-8") as config_file:
            raw = yaml.load(config_file, Loader=UniqueKeySafeLoader)
    ...
    raw = deepcopy(raw)
    base_reference = raw.pop("extends", None)
    raw = _normalize_declared_paths(raw, path.parent)
    if base_reference:
        base_path = Path(str(base_reference)).expanduser()
        if not base_path.is_absolute():
            base_path = path.parent / base_path
        base_path = base_path.resolve()
        base = _load_config_mapping(base_path, (*stack, path))
...
def load_mujoco_config(config_path: str) -> Dict[str, Any]:
    ...
    normalized = _normalize_declared_paths(config, path.parent)
    normalized["arm_config"] = dict(normalized.get("arm_config") or {})
...
def validate_mujoco_config(config: Mapping[str, Any]) -> None:
    ...
    ik_backend = str(arm_config.get("ik_backend", "ros_service")).strip().lower()
    if ik_backend not in {"ros_service", "marvin_local"}:
        raise MujocoConfigError(
            "arm_config.ik_backend must be ros_service or marvin_local"
        )
    if ik_backend == "marvin_local":
        for key in (
            "marvin_ik_module_path",
            "marvin_ik_library_path",
            "marvin_ik_config_path",
        ):
            value = str(arm_config.get(key, "")).strip()
            if not value:
                raise MujocoConfigError(f"arm_config.{key} is required")
        arm_type = _require_integer(
            arm_config.get("marvin_arm_type", 1),
            "arm_config.marvin_arm_type",
        )
        if arm_type not in (0, 1):
            raise MujocoConfigError("arm_config.marvin_arm_type must be 0 or 1")
        zsp_type = _require_integer(
            arm_config.get("marvin_zsp_type", 0),
            "arm_config.marvin_zsp_type",
        )
        if zsp_type not in (0, 1):
            raise MujocoConfigError("arm_config.marvin_zsp_type must be 0 or 1")
        _require_vector(
            arm_config.get("marvin_zsp_para", [0.0] * 6),
            "arm_config.marvin_zsp_para",
            6,
        )
        _require_finite_number(
            arm_config.get("marvin_zsp_angle_deg", 0.0),
            "arm_config.marvin_zsp_angle_deg",
        )
        _require_positive(
            arm_config.get("marvin_translation_units_per_meter", 1000.0),
            "arm_config.marvin_translation_units_per_meter",
        )
        _require_positive(
            arm_config.get("marvin_max_ik_joint_jump_rad", 0.75),
            "arm_config.marvin_max_ik_joint_jump_rad",
        )
    service_ns = str(
